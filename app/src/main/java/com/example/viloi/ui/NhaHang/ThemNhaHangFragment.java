...
public class ThemNhaHangFragment extends Fragment {
    ...
    private void validate() {
        ...
        if (sdt.isEmpty()) { edtPhone.setError("Bắt buộc"); return; }
        ...
        if (selectedImageUri != null) {
            ...
            luuFirestore(new ArrayList<>());
        }
    }

    private void uploadAnhRoiLuu() {
        String fileName = "nha_hang/" + UUID.randomUUID() + ".jpg";
        StorageReference ref = storage.getReference().child(fileName);
        ref.putFile(selectedImageUri)
                .addOnSuccessListener(task ->
                        ref.getDownloadUrl().addOnSuccessListener(uri -> {
                            List<String> anhList = new ArrayList<>();
                            anhList.add(uri.toString());
                            luuFirestore(anhList);
                        }))
                .addOnFailureListener(e -> {
                    // Lưu không ảnh nếu upload lỗi
                    luuFirestore(new ArrayList<>());
                });
    }
    ...
    private void luuFirestore(List<String> anhList) {
        ...
        db.collection("nha_hang")
                ...
                .addOnSuccessListener(ref -> {
                    if (!isAdded()) return;
                    Toast.makeText(getContext(), "Đã thêm nhà hàng ✓",
